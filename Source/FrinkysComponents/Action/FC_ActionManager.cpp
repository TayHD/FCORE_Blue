...
bool UFC_ActionManager::SpawnAndAddAction(TSubclassOf<AFC_Action> ActionToAdd, UObject* Payload)
{
	...
	if(!ActionToAdd.GetDefaultObject()->bAllowDuplicates)
	{
		if(DoesActionExist(ActionToAdd))
		{
			return false;
		}
...
bool UFC_ActionManager::DoesActionExist(TSubclassOf<AFC_Action> ActionToAdd)
{
...
void UFC_ActionManager::EvaluateActions()
{
	...
	if(IsValid(ActionToEvaluate.Action) && OwningPawn)
	{
		...
		{
			ExecuteAction(ActionToEvaluate.Action, EActionExecutionContext::ExecutingLocal, ActionToEvaluate.Payload);
...
void UFC_ActionManager::ExecuteAction(TSubclassOf<AFC_Action>& ActionToExecute, EActionExecutionContext ExecutionContext, UObject* Payload)
{
	...
	SpawnedAction->ActionExecutionContext = ExecutionContext; // set the execution context
	
	if(!SpawnedAction->CanExecute(GetOwner(), Payload)) // if we can't execute the action, we end it
	{
		SpawnedAction->EndAction();
		return;
	}
	
	SpawnedAction->OnExecute(ExecutionContext, Payload); // execute the action
	OnActionStarted.Broadcast(ActionToExecute); // broadcast that the action has started
	
	switch (ExecutionContext)
	{
	case EActionExecutionContext::ExecutingLocal:
		SpawnedAction->OnExecuteLocal(Payload);
		break;
	case EActionExecutionContext::ExecutingRemote:
		SpawnedAction->OnExecuteRemote(Payload);
		break;
	case EActionExecutionContext::ExecutingOnServer:
		SpawnedAction->OnExecuteServer(Payload);
		break;
	}
}
...
{
	ExecuteAction(ActionToExecute, EActionExecutionContext::ExecutingOnServer, Payload);
...
void UFC_ActionManager::MultiExecuteAction_Implementation(TSubclassOf<AFC_Action> ActionToExecute, UObject* Payload)
{
	if(OwningPawn && !OwningPawn->IsLocallyControlled())
	{
		ExecuteAction(ActionToExecute, EActionExecutionContext::ExecutingRemote, Payload);
	}
}
