...
void UFC_DynamicIconSubsystem::InitSceneCapture(UFC_DynamicIconComponent* DynamicIconComponent)
{
	...
	DynamicIconComponent->GetOwner()->GetAttachedActors(AttachedActors);
	for (AActor* AttachedActor : AttachedActors)
	{
		SceneCaptureComponent->ShowOnlyActors.Add(AttachedActor);
	}
...
void UFC_DynamicIconSubsystem::GenerateDynamicIcon(FGenerateDynamicIconTask &Task)	
{
	...
	bool ActorHiddenInGame = DynamicIconComponent->GetOwner()->IsHidden();
	DynamicIconComponent->GetOwner()->GetRootComponent()->SetVisibility(true, true);
	DynamicIconComponent->GetOwner()->GetRootComponent()->MarkRenderStateDirty();

	// -- Save and Set Actor Collision
	bool ActorCollision = DynamicIconComponent->GetOwner()->GetActorEnableCollision();
	DynamicIconComponent->GetOwner()->SetActorEnableCollision(true);
	...
	// draw a debug box showing the bounds
	//DrawDebugSolidBox(GetWorld(), BoundingBox.GetCenter(), BoundingBox.GetExtent(), FQuat(FRotator::ZeroRotator), FColor::Red, false, 1000.0f, 0);
	if(DynamicIconComponent->bAutoOrient) {
		BoundingBox.GetExtent().X < BoundingBox.GetExtent().Y ? bUseXForward = true : bUseXForward = false;
		FMath::Max(BoundingBox.GetExtent().X, BoundingBox.GetExtent().Y) < BoundingBox.GetExtent().Z ? bFlipHorizontal = true : bFlipHorizontal = false;
	...
	// -- Reset Visibility
	DynamicIconComponent->GetOwner()->GetRootComponent()->SetVisibility(ActorHiddenInGame, true);

	// -- Reset Actor Collision
	DynamicIconComponent->GetOwner()->SetActorEnableCollision(ActorCollision);
