...
UFC_GenericAttachablePoint* UFC_GenericAttachableHandler::FindSlotByTag(FGameplayTag Tag)
{
	for (UFC_GenericAttachablePoint* Slot : Slots)
	{ 
		if(Slot->Data.Tag.MatchesTag(Tag) || Tag.MatchesTag(Slot->Data.Tag))
		{
			return Slot;
		}
	}
...
{
	return IsValid(Attachable) && IsValid(SlotToCheck) && (Attachable->SlotTag.MatchesTag(SlotToCheck->Data.Tag) || SlotToCheck->Data.Tag.MatchesTag(Attachable->SlotTag));
}
