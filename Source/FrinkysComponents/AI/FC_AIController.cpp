...
void AFC_AIController::UpdateControlRotation(float DeltaTime, bool bUpdatePawn) 
{
	...
	{
		NewControlRotation = FMath::RInterpTo(MyPawn->GetControlRotation(), (FocalPoint - MyPawn->GetPawnViewLocation()).Rotation(), DeltaTime, AimInterp); 
	}
...
FVector AFC_AIController::GetFocalPointOnActor(const AActor* Actor) const
{
	const ACharacter* Char = Cast<ACharacter>(Actor);
	if(Char)
	{
		return Char->GetMesh()->GetSocketLocation(FName(AimAt));
	}
