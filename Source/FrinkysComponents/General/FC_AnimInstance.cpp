...
void UFC_AnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	if(Pawn)
	{
		...
		RelativeVelocity = Pawn->GetActorRotation().UnrotateVector(Velocity);
		Direction = UKismetMathLibrary::DegAtan2(RightVelocity, ForwardVelocity);
...
void UFC_AnimInstance::MultiSetControlRotation_Implementation(FRotator Rotation)
{
	...
	{
		float ViewAngle = Pawn->RemoteViewPitch * 360.f/255.f;
