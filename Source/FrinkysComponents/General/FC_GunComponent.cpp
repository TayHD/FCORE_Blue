...
void UFC_GunComponent::FireStopClient_Implementation()
{
	FireStop();
}
...
void UFC_GunComponent::ClientFire_Implementation()
{
	if(OwningPawn)
	{
		if (Ammo > 0)
		{
			...
			ServerHandleSurfaceImpacts(Hits);
			OnClientHit.Broadcast(Hits);
		
			if (OwningPawn->IsLocallyControlled())
			{
				HandleSurfaceImpacts(Hits);
...
{
	// change barrel, only if we have authority
...
void UFC_GunComponent::OnRep_FireMode()
{
	OnFireModeChange.Broadcast();
}
...
void UFC_GunComponent::ServerHandleSurfaceImpacts_Implementation(const TArray<FCustomHitData>& HitResults)
{
	MultiHandleSurfaceImpacts(HitResults);
}

void UFC_GunComponent::HandleSurfaceImpacts(const TArray<FCustomHitData>& HitResults)
{
	if(IsValid(SurfaceData))
	{
		for (auto HitResult : HitResults)
		{
			FSurfaceData DataFromHitResult = SurfaceData->GetDataFromHitResult(HitResult.HitResult);
			
			if(DataFromHitResult.Sound)
			{
				UGameplayStatics::PlaySoundAtLocation(GetWorld(), DataFromHitResult.Sound, HitResult.HitResult.ImpactPoint + HitResult.HitResult.Normal * 10, 1, 1, 0, ImpactAttenuation);
			}
			
			UNiagaraSystem* NiagaraEmitter = DataFromHitResult.NiagaraEmitter;

			// If we have a niagara emitter, spawn it
			if(NiagaraEmitter)
			{
				UNiagaraFunctionLibrary::SpawnSystemAtLocation(GetWorld(), NiagaraEmitter, HitResult.HitResult.ImpactPoint + HitResult.HitResult.Normal * 10, HitResult.HitResult.ImpactNormal.Rotation());
			}
		}
	}
}

void UFC_GunComponent::MultiHandleSurfaceImpacts_Implementation(const TArray<FCustomHitData>& HitResults)
{
	if (OwningPawn && !OwningPawn->IsLocallyControlled())
	{
		HandleSurfaceImpacts(HitResults);
	}
}
